...
const filteredOrders = computed(() => {
  ...
  if (selectedOption.value && selectedOption.value !== 'All Order') {
    filtered = filtered.filter(order => order.status === selectedOption.value)
  }
    ...
    <main class="ml-[14rem] w-full py-4 px-8 flex flex-col gap-4 bg-gray-50 h-screen">
      ...
      <div class="relative inline-block mt-2">
        ...
        <ul v-if="dropdownVisible" class="absolute bg-gray-80 rounded-lg shadow-lg text-gray-500">
          <li
            v-for="option in ['Pending', 'Preparing', 'In delivery', 'Delivered', 'Cancelled', 'All Order']"
            :key="option"
